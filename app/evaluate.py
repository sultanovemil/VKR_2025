...
ANNOTATIONS_FILE = 'test/test_annotations.json'
...
for item in tqdm(annotations['images'], desc="Оценка качества распознавания"):
    ...
    recognized_texts = process_image(image_path, model)
...
# Общая статистика
total_chars = df_stats['Всего символов'].sum()
...
print(f"Общий CER: {overall_cer:.4f}")
print(f"Общая точность: {1 - overall_cer:.2%}")